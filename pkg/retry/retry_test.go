...
func TestDoOneParam(t *testing.T) {
	type args struct {
		ctx              context.Context
		repeat           int
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := DoOneParam(tt.args.ctx, tt.args.repeat, tt.args.retryFunc, tt.args.p, tt.args.isRepeatableFunc); (err != nil) != tt.wantErr {
...
func TestDo(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			if err := Do(tt.args.ctx, tt.args.repeat, tt.args.retryFunc, tt.args.isRepeatableFunc); (err != nil) != tt.wantErr {
...
func ExampleDoOneParam() {
	err := DoOneParam(context.Background(), 3, oneParamR, 1, IsRetryable)
	if err != nil {
		fmt.Print(err.Error())
	}
